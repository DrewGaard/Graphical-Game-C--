...
void Game::setUpGame()
{
	...
	//In this case it's a portal
	r = rand()%(rows-1) + 1;
	c = rand()%(cols-1) + 1;
	world[r][c] = new Fort('F');

	for (int i = 0; i < 4; i++) //Place four crystals in the world
	{
		do
		{
			r = rand() % (rows - 1) + 1;
	...
	for (int i = 0; i < 3; i++) //Place three shields in the world
	{
		do
		{
			r = rand() % (rows - 1) + 1;
		...
		world[r][c] = new Shield('S');
	}

	for (int i = 0; i < 1; i++) //Place shops in the world
	{
		do
		{
			r = rand() % (rows - 1) + 1;
		...
		world[r][c] = new Shop('G');
	}

	for (int i = 0; i < 15; i++) //Place 15 monsters in the world
	{
		do
		{
			r = rand() % (rows - 1) + 1;
		...
		world[r][c] = new Monster('M');
	}

	//TODO: Put objects of your own Location subclasses into the world
