		...
		case SDL_KEYDOWN:
		{
			...
			else if (theEvent->key.keysym.sym == SDLK_i)
			{
				...
				if (totalItems > 0)	//Add the code to see if the user wants to drop an item
				{
					...
					{
						const int MAX_INVENTORY_SIZE = 20;							//Make this the max number of items the user will be able to pickup in the game plus one, becasue of the cancel option
...
void Videogame::instructions()
{
	...
	message += "\n-----------PLEASE USE THE MOUSE TO SELECT THE COMBAT OPTIONS DURING THE GAME!-----------";
